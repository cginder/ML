...
```{r}
# Create a for loop with p ranging from 2 to 20 to create p-1 linear regression models

```

**1.8.4 Create (p-1)*(k -1) k-NN models [p ranges from 2 to 20 and k ranges from 2 to 15]**

```{r}
# Create a for loop with p ranging from 2 to 20 and k ranging from 2 to 15 to create (p-1)*(k-1) iterations of a k-NN function

```

**1.8.5 Create p-1 plots, with MSE on the y-axis, log(1/k) on the x-axis.** 

Each of the p-1 charts will have: 1) set of k-NN data points representing the MSE at different values of K and 2) MSE of linear regression model. Goal is to see how the # of noisy variables affects k-NN and linear regression differently
...
## need to fix this so that the pfit values correspond to the ordered mileage data points
```
